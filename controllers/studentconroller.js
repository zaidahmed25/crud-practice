...
const addStudent = async (req, res) => {
    const { name, email, age} = re.body;
    try {
        const student = await Student.create({ name, email, age });
        res.status(201).send({ messgae: "student added successfully"})
    } catch (error) {
        res.status(500).send({ messgae: "email should be unique", error: error });
    }
}
...
const getStudent = async (req, res) => {
    try {
        ...
        res.status(404).send({ messgae: 'error occurred'})
    }
}
...
const getSingleStudent = async (req, res) => {
    ...
    if (!student) {
        request.status(404).send({ messgae: 'user not found'});
        return;
    }
    response.status(200).send({ student: student })
}
